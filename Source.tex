...
In this section, we introduce rewriting strategies over vector spaces. We
formulate a confluence criterion in terms of strategies as well as a 
method based on strategies to construct linear bases of vector spaces. We 
also obtain a new proof of the diamond's lemma.
\medskip
...
In this section, we introduce rewriting strategies over vector spaces,
and we construct a normalisation operator associated to a strategy. This
normalisation operator is used in
...
subset $R$ of $\basis\times V$, such that for every $(e,v)\in R$, we 
have~$e\notin\supp(v)$. Elements of $R$ are called {\em rewriting rules}
and are denoted by $e\rewR v$ instead of~$(e,v)$. We extend these rules
into a rewriting relation on $V$, still written $\rewR$, with rewriting
steps of the following form:  
...
In the statement of Proposition~\ref{prop:vs_structure}, we denote by
$V(R)$ the subspace of $V$ spanned by elements of the form~$e-v$, where
$e\rewR v$ is a rewriting rule.
\smallskip

\begin{proposition}\label{prop:vs_structure}
  There is a bijection between the quotient set $V/\equivR$ and the
  quotient vector space~$V/V(R)$.
\end{proposition}

\begin{proof}
  The bijection maps every equivalence class $[v]_{\rewEquiv}$ to
  $v+V(R)$. Hence, the statement of the proposition means that
  $v_1\equivR v_2$ holds if and only if $v_1-v_2\in V(R)$. The
  implication from left to right follows from the facts that
  $v_1\rewR v_2$ implies $v_1-v_2\in V(R)$ and that the binary relation
  on~$V$ defined  by~$v_1-v_2\in V(R)$ is an equivalence relation. For
  the converse implication, let us write $v_1-v_2=\sum\mu(e-v)$, where
  $\mu$ are scalars and $e\rewR v$ are rewriting rules. Since
  $e\equivR v$ holds for each term of the sum, Lemma~\ref{lem:butterfly}
  implies that $v_1\equivR v_2$, which concludes the proof.
\end{proof}
\smallskip
...
Moreover, Proposition~\ref{prop:vs_structure} is not true if we do not
assume that $e\notin\supp(v)$ for every rewriting rule~$e\rewR v$: for
instance, if we have only one rule $e\rewR 2e$, then $e\in V(R)$ but
$[e]_{\equivR}\neq[0]_{\equivR}$. Notice that we explicit used the
assumption $e\notin\supp(v)$ in the proof of Lemma~\ref{lem:butterfly}.
\smallskip

Now, we introduce rewriting strategies. We fix a subset~$S$ of~$R$ with
pairwise distinct left-hand sides, that is, given two rules $e\rewR v$
and $e'\rewR v'$ in $S$ such that $e=e'$, we have $e\rewR v=e'\rewR v'$.
Let us consider the endomorphism $r_S:V\to V$ defined by $r_S(e)=v$ if
there exists a rule $e\rewR v$ in $S$ with left-hand side $e$, and
$r_S(e)=e$, otherwise. We consider a new binary relation $\parS$ on $V$
defined by $v\parS v'$ whenever $v\neq r_S(v)$ and $v'=r_S(v)$. In the
sequel, we refer this relation as being the parallel rewriting relation
induced by $S$; the terminology parallel makes explicit that all basis
vectors in $v$ are reduced at once with rules in $S$ and justifies the
double head arrows in $\parS$. Notice that by definition, and contrary to
$\rewR$, the parallel rewriting relation is stable under vector space
operations. Let us denote by~$\ordS$ the rewriting preorder on~$\basis$
induced by~$\parS$, that is, $\ordS$ is the transitive closure of the
relation: there exists a vector~$v$ such that $e'\transParS v$ and
$e\in\supp(v)$. In other words, $e\ordS e'$ if and only if
$e\in\supp(r_S^n(e'))$, where $n$ is a nonnegative integer and $r_S^n$ is
the $n$-th composition of $r_S$. 
\smallskip
...
$\leq_R$ of $R$ is terminating, then any pre-strategy for $R$ is also a
strategy since $e\ordS e'$ implies $e\leq_Re'$.
\smallskip
...
\begin{example}\label{ex:strategies_step_1}
  ...
  \item\label{it:ex_strat_1} Consider the $4$-dimensional vector space
    ...
    strategy and $\parS$ is terminating. Notice that any
    $v=\lambda_1e_1+\cdots+\lambda_4e_4$ admits a unique normal form for
    $\parS$ that is computed as follows:
