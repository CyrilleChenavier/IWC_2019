...
\begin{proof}
  ...
  $u \rightsquigarrow_R^* 0$ by Proposition \todo{ref}. By the previous result $\SNF(u) = \SNF(0) = 0$ and hence we get the inclusion $(R) \subseteq \ker(\SNF)$. 
...
\begin{definition}\label{def:decreasing}
  Given a strategy $S$ for $R$ and a terminating order $\prec$ on $R$ and a rule $f = (e,v) \in R$, we denote by $R_{\prec f}$ the set of  the  we
...
\begin{proof}
  ...
  \item If $e$ is an $S$-normal form, then we have $\SNF(e)=e$ and
    ...
    e\ar[d, equal]\\
    v\ar[r, "*", leftrightarrow, squiggly, "_S"'] & e.
    \end{tikzcd}\]
