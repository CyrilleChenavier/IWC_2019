...
is caracterised in terms of a decreasingness property with respect to a
well-founded order on rewriting rules. Moreover, this characterisation
also shows that even if the notion of $S$-confluence seems to be strict
at a first sight, it is flexible in the sense that allowing to reverse
rewriting rules does not bring more confluence diagrams. In the statement
of the proposition, we use the following notation: for every basis
element $e$, we write $e\overset{=}{\twoheadrightarrow}_Sr_e$, for
$e\rewS r_e$ if $e$ is not minimal and $r_e=e$, otherwise.
\smallskip
...
\begin{proof}
  ...
  \item If $e$ is not a minimal basis element and if the rule
    $e\rewR r$ belongs to $S$, then we have the following diagram:
    \[\begin{tikzcd}
  ...
  \item In the last case, $e$ is not minimal and the rule $e\rewR r$ does
    not belong to $S$. First, notice that by definition of a strategy, 
    there exists a rewriting rule $e\rewS r_e$ in $S$. Moreover, by
    definitions of the map $\SNF$ and of the $S$-confluence property, we
    have the equalities $\SNF(r_e)=\SNF(e)=\SNF(r)$. Hence, we get the
    following diagram:
    \[\begin{tikzcd}
    ...
    than $e\rewR r$ for the order $\prec$ by definition. Hence, the
    decreasingness property that we expect holds.
  \end{itemize}
...
\begin{remark}
  ...
  (see~\cite[Definition 3]{van2008confluence}), using the order $\prec$
  on $R$ and the discrete ordering on $\rewS$. 
...
\begin{example}\label{ex:end_to_example}
  ...
  for the ordering $\prec$. The decreasing diagrams are the following:
  \[\begin{tikzcd}
