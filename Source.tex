...
\begin{example}\label{ex:h_norma_form}
  We let $X=\{x,y,z,t\}$, $x\rewR z$, $x\rewR y$, $y\rewR z$ and
  $z\rewR t$. We choose the order $x>z$ $y>z$, $h_x=x\rewh z$ and
  $h_y=y\rewh z$. Then $z$ is a $h$-normal form but is not a normal form
  for $\rewR$.
\end{example}
...
\begin{example}
  We let $X=\{x,y,z,t\}$, $x\rewR z$, $x\rewR y$ and $y\rewR z$. We
  choose the order $x>z$ $y>z$, $h_x=x\rewh z$ and $h_y=y\rewh z$. We
  have $H(x)=H(y)=H(z)=z$, so that $R$ is $h$-confluent.
  However, if we had the rewrite rule $z\rewR t$ as in
  Example~\ref{ex:h_norma_form}, we have $H(z)=z$ and $H(t)=t$, so that
  $R$ is not $h$-confluent.
\end{example}
