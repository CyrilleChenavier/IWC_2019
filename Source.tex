...
\begin{proof}
  ...
  inverses to each other, which concludes the proof. \todo{Le résultat
    est faux si on a des règles de la forme $v\parS v$. Est-ce qu'on
    s'emmerde à expliquer ça?}
\end{proof}

Notice that if $r$ is the function corresponding to $\parRew$, then we 
have $v\transRew v'$ if and only if $r(v)\neq v$ and there exists an
integer $n$ such that $v'=r^n(v)$, where $r^n$ denotes the $n$-th
composition of the function~$r$, seen as a linear map on $V$.
\medskip
...
that is, $e\ordS e'$ if there exists a vector $v$ such 
...
\begin{example}\label{ex:strategies_step_1}
  ...
  \item\label{it:case_N} Let $V$ be the vector space with basis
    ...
    $k \in \mathbb N$ such that $n + k \notin E$. In other words, $S$ is
    a strategy if and only if the complement of $E$ in $\mathbb N$ is
...
\begin{proof}
  ...
  \[v\transS\sum\lambda_ie_i+\SNF(v')\transS\sum\lambda_ir_S(e_i)+r_S(v')
  =r_S(v).\]
...
$\rewR$ is confluent relative to this strategy. \todo{Éventuellement
  prendre l'autre contre-exemple}
\smallskip
...
Proposition~\ref{prop:S-conf_decreasing}. \todo{vérifier}
