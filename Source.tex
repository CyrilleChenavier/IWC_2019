...
Finally in Section \ref{sec:link_with_classical} we relate our theory to the classical one.
...
still written $\rightsquigarrow_R$, with rewriting steps are of the following form: 
\begin{equation}\label{equ:R_rewriting_step}
  \lambda e+u\rightsquigarrow_R \lambda v+u,
  \smallskip
\end{equation}
where $e\rewR v\in R$ is a rewriting rule, $\lambda$ is a nonzero scalar
and $u$ is a vector. Finally, we denote by $(R)$ the subspace of $V$ spanned by elements of the form $e - v$, for $(e,v) \in R$. 

\begin{proposition}\label{prop:squig_to_quotient}
  For any two $u,v \in V$, the following are equivalent:
  \begin{enumerate}
  \item $u \rightsquigarrow^*_R v$
  \item $u \leftrightsquigarrow^*_R v$
  \item $\bar u = \bar v$ in the quotient $V/(R)$.
  \end{enumerate}
\end{proposition}
\begin{proof}
  The fact that $(1) \Rightarrow (2) \Rightarrow (3)$ is immediate. As for $(3) \Rightarrow (1)$, if $\bar u = \bar v$ then there exists $n \mathbb N$ and a family $(e_i,r_i) \in R$ for $i \leq n$, and scalars $\lambda_i$ such that $u - v = \sum_{i=1}^n \lambda_i(e_i - r_i)$.  As a result, we have:
  \[
    u = \sum_{i = 1}^n \lambda_i(e_i - r_i) + v \rightsquigarrow_R \sum_{i=1}^{n-1} \lambda_i (e_i-r_i)  + v \rightsquigarrow_R \ldots \rightsquigarrow_R v,
  \]
  where at each step we use the relation $e_i \rightsquigarrow_R r_i$. This proves finally that $u \rightsquigarrow^* v$.
\end{proof}
The previous proposition show in particular that~$\rightsquigarrow^*_R$ is
stable under vector space operations, that is if $u_1\rightsquigarrow^*_R u_2$
and $v_1\rightsquigarrow_R v_2$, then for any $\mu\in\K$,
$\mu u_1+v_1\rewR \mu u_2+v_2$.

Let $S$ be a subset of~$R$ with pairwise distinct left-hand sides. Then $S$
induces a map $r_S : R \to R$ defined on $\basis$ by $r_S(e) = r$ if
$(e,r) \in S$, and $r_S(e) = e$ if there is no such $r$. We define $\parS$ as the
relation $u \parS v$ whenever $v = r_S(u)$.  Let us denote
by~$\ordS$ the rewriting preorder on~$\basis$ induced by~$\parS$, that is,
$\ordS$ is the transitive closure of the relation: there exists a vector $v$
such that $e'\transParS v$ and~$e\in\supp(v)$. In other words, $e\ordS e'$ if and
only if $e\in\supp(r_S^n(e'))$, for some nonnegative integer~$n$.  \smallskip
...
\begin{definition}\label{def:strategies}
  ...
  such that the rewriting preorder $\ordS$ associated to $\parS$ is terminating. 
\end{definition}
\smallskip
...
$\leq_R$ is terminating, then any pre-strategy is a strategy since
$e\ordS e'$ implies $e\leq_Re'$.
\smallskip

\begin{example}\label{ex:strategies_step_1}\todo{Adapt this}
...
\begin{proposition}\label{prop:par_to_squigequiv} The following hold:
  \begin{itemize}
  \item $\SNF$ is a linear projector.
  \item If $u \parS v$, then $u \leftrightsquigarrow^*_S v$.
  \end{itemize}
\end{proposition}
\begin{proof}
  For the first point, take $u,v \in V$ and $\lambda \in \K$. By termination, there exists $n,m \in \mathbb N$ such that $r^n_S(u) = r^{n+1}_S(u)$ and $r^m_S(v) = r^{m+1}_S(v)$. Taking $k \geq m,n$, we have
  \[
    r^{k+1}_S(u+ \lambda v) = r^{k+1}_S(u) + \lambda r^{k+1}_S (v) = r^n_S (u) + \lambda r^m_S (v) = \SNF(u) + \lambda \SNF(v).
  \]
This proves that $\SNF$ is linear. To see that it is a projector, notice that $r_S(\SNF(u)) = r^{n+1}_S(u) = \SNF(u)$, hence $\SNF(\SNF(u)) = u$.

  For the second point, if $u \parS v$ then $\bar u = \bar v$ in $V/(S)$ and hence $u \leftrightsquigarrow^*_S v$ by the previous proposition.
\end{proof}
...
\begin{theorem}\label{thm:S-confluence_criterion}
  Let $R$ be a set of rewriting rules and let $S$ be a strategy for $R$.  If the
  rewriting relation~$\rewR$ is $S$-confluent, then for all $u,v$ such that $u \leftrightsquigarrow_R^* v$, $\SNF(u) = \SNF(v)$. As a result, we have a vector spaces
  isomorphism
  \begin{equation}\label{equ:iso_SNF}
    V/(R)\quad\simeq\quad\im(\SNF).
  \end{equation}
...
\begin{proof}
  For the first claim, since having equal $S$-normal forms is an equivalence
  relation on $V$, we only have to check that whenever $u \rightsquigarrow_R v$,
  $\SNF(u) = \SNF(v)$. Let $(e,r) \in R$, $u \in V$ and $\lambda \in \K$. By hypothesis $\SNF(e - r) = 0$ and hence by linearity of $\SNF$,
  \[
    \SNF(\lambda e + u) = \lambda \SNF(e) + \SNF(u) = \lambda \SNF(r) + \SNF(u) = \SNF(\lambda r + u). \]
  
  As for the second claim, we need to show that $(R) = \ker(\SNF)$. The inclusion
  $\ker(\SNF) \subseteq (R)$ is immediate. Reciprocally if $u \in (R)$, then
  $u \rightsquigarrow_R^* 0$ by Proposition \ref{prop:squig_to_quotient}. By the previous result $\SNF(u) = \SNF(0) = 0$ and hence we get the inclusion $(R) \subseteq \ker(\SNF)$. 
...
\begin{definition}\label{def:decreasing}
  Given a strategy $S$ for $R$ and a terminating order $\prec$ on $R$ and a rule $f = (e,v) \in R$, we denote by $R_{\prec f}$ the set of elements of $R$ stricly smaller than $f$.  We
  say that $R$ is {\em decreasing} w.r.t.\ $(S,\prec)$ if for every
  rewriting rule $f = e\rewR v$, we have a diagram:
  \[\begin{tikzcd}
      e\ar[d, "_R"']\ar[r, twoheadrightarrow, "=", "_S"'] &
      r_S(e)\ar[d, leftrightsquigarrow, dotted, "*"', "_{R_{\prec f}}"]\\
      v\ar[r, leftrightsquigarrow, dotted,  "*", "_{R_{\prec f}}"'] & u
    \end{tikzcd}\]
...
\begin{proof}
  ...
  \item Otherwise, by definition of
    the map $\SNF$ and of the $S$-confluence property, we have the
    equalities
    \[\SNF(r_S(e))=\SNF(e)=\SNF(v).
    \smallskip\]
    By Proposition~\ref{prop:par_to_squigequiv}, we get the following
    diagram:
    \[\begin{tikzcd}
    e\ar[d, "_R"']\ar[r, twoheadrightarrow, "_S"'] &
    r_S(e) \ar["_{S}", "*"', squiggly, leftrightarrow, d] \\
    v \ar[r, "_{S}"', squiggly, leftrightarrow, "*"] & \SNF (e).
  \end{tikzcd}\] Hence, the diagram is decreasing w.r.t.\ $(S,\prec)$.
\end{itemize}
  ...
  \[\begin{tikzcd}
  e\ar[d, "_R"']\ar[r, twoheadrightarrow, "=", "_S"'] &
  r_S(e)\ar[d, leftrightarrow, squiggly, dotted, "*"', "_{R_{\prec f}}"]\\
  v\ar[r, leftrightarrow, squiggly, dotted, "*", "_{R_{\prec f}}"'] & u
  \end{tikzcd}\]
...
\subsection{Link with classical linear rewriting}
\label{sec:link_with_classical}

In this section, we suppose that for any $(e,r) \in R$, $e \notin \supp(r)$. We extend $\rewR$ into a rewriting relation on $V$,
...
we refer to~$\rewR$ as the rewriting relation induced by the rewriting rules
...
\begin{proposition}\label{prop:vs_structure}
  The relations $\equivR$ and $\leftrightsquigarrow_R^*$ coincide.
\end{proposition}
\begin{proof}
  The fact that $\rewR$ is a subrelation of $\rightsquigarrow_R$ implies the inclusion $\equivR \subseteq \leftrightsquigarrow_R^*$. For the other inclusion, take a relation $(e,r) \in R$, $u \in V$ and $\lambda \in \K$, and let us show that $\lambda e + u \equivR \lambda r + u$.
  Let us write $u = \mu e + u'$, with $e \notin \supp(u')$. Then we have:
  \[
\lambda e + u = (\lambda + \mu) e + u' \rewR (\lambda + \mu) r + u'.
\]
Additionally by hypothesis $e \notin \supp(r)$ and so $e \notin \supp(\lambda r + u')$, which yields:
\[
  \lambda r + u = \mu e + (\lambda r + u) \rewR \mu r + (\lambda r + u) = (\mu + \lambda) r + u.
\]
Hence we get $\lambda e + u \equivR \lambda r + u$. This proves the inclusion $\rightsquigarrow_R \subseteq \equivR$ and hence $\leftrightsquigarrow_R^* \subseteq \equivR$. 
\end{proof}

\begin{proposition}
  If $S$ is a strategy for $R$, then we have the inclusion
  \[ \parS \subseteq \transR. \]
\end{proposition}
\begin{proof}
 Since $S$ is a strategy, the
...
\begin{proposition}
If $R$ is $S$-confluent, then it is confluent.
\end{proposition}
\begin{proof}
  We will show the $R$ satisfies the Church-Rosser property. Take $u,v \in V$ such that $u \equivR v$. In particular $u \leftrightsquigarrow_R^* v$, and hence by $S$-confluence $\SNF(u) = \SNF(v)$. Since $u \parS \SNF(u)$ and $v \parS \SNF(v)$, by the previous proposition we get $u \transR \SNF(u)$ and $v \transR \SNF(v)$, which proves the Church Rosser property.
\end{proof}
