...
$u$ such that $e'\transParS u$ and $e\in\supp(u)$. In other words,
...
another vector $u'$ such that $u\transParS u'$ and $r_S$ acts trivially 
...
\begin{example}\label{ex:strategies_step_1}
  ...
  \item\label{it:case_N} Let $V$ be the vector space with basis
    ...
    Moreover, such a pre-strategy is a strategy if and only if for all
...
We finish this section with two other results: we first show in
Proposition~\ref{prop:S-conf_decreasing} that $S$-confluence is
caracterised in terms of the decreasingness property that we introduce in
Definition~\ref{def:decreasing}, and from this, we give a new proof of
the diamond's lemma in Theorem~\ref{thm:diamond_lemma}.
\smallskip

\begin{definition}\label{def:decreasing}
  Given a strategy $S$ for $R$ and a Noetherian order $\preceq$ on $R$,
  we say that $R$ is {\em decreasing} w.r.t.\ $(S,\preceq)$ if for every
  ...
  smaller than $e\rewR v$ relative to $\preceq$.
\end{definition}
\smallskip
...
\begin{proposition}\label{prop:S-conf_decreasing}
  ...
  \item There exists a Noetherian order $\preceq$ on $R$ such that $R$ is
    decreasing w.r.t.\ $(S,\preceq)$.
  \end{enumerate}
\end{proposition}

\begin{proof}
  $(1)\Rightarrow (2)$: Let $\preceq$ be the order on $R$ whose
  corresponding strict order $\prec$ is defined, for every pair of rules
  $e\rewR r_S(e)\in S$ and $e'\rewR v\notin S$, by
  $e\rewR r_S(e)\prec e'\rewR v$. This order is Noetherian since each
  chain of strictly decreasing elements has length $2$. Let $e\rewR v$ be
  a rewriting rule and let us construct a decreasing diagram as in
  Definition~\ref{def:decreasing}. We distinguish two cases.
  \begin{itemize}
  \item If $e\rewR v=e\parS r_S(e)$ belongs to $S$, then we have the 
    ...
    diagram obviously is decreasing w.r.t.\ $(S,\preceq)$.
  \item Otherwise, by definition of the map $\SNF$ and of fropm the
    $S$-confluence hypothesis, we have the equalities:
    \[\SNF(r_S(e))=\SNF(e)=\SNF(v).\smallskip\]
    From Lemma~\ref{lem:strategies}, we get the following diagram:
    \[\begin{tikzcd}
    e\ar[d, "_R"']\ar[r, twoheadrightarrow, "_S"'] &
    r_S(e) \ar["_{R}", "*"', leftrightarrow, d] \\
    v \ar[r, "_{R}"', leftrightarrow, "*"] & \SNF (e).
    \end{tikzcd}\]
    Moreover, this diagram can be chosen in such a way that each rule in
    the right and bottom faces belong to $S$, so that it is decreasing
    \wrt\ $(S,\preceq)$.
\end{itemize}

  $(2)\Rightarrow (1)$: Let $e\rewR v$ be a rewriting rule and let us
  assume by induction that for every other rule~$e'\rewR v'$ strictly
  smaller than $e\rewR v$ for $\preceq$, we have $\SNF(e')=\SNF(v')$.
  Consider a decreasing diagram:
  \[\begin{tikzcd}
  e\ar[d, "_R"']\ar[r, twoheadrightarrow, "_S"'] &
  r_S(e)\ar[d, leftrightarrow, "*"', "_{R}"]\\
  v\ar[r, leftrightarrow, "*", "_{R}"'] & u
  \end{tikzcd}\]
  ...
  $\preceq$ being Noetherian, this inductive argument proves that
