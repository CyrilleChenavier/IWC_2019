...
preorder $\ordS$ of $S$ as being the (strict) preorder on~$\basis$
defined by $e\ordS e'$ if there exists a strictly positive integer $n$
such that $e\in\supp(r_S^n(e'))$, where $r_S^n$ is the $n$-th composition
of $r_S$. 
\medskip
...
rewriting preorder of $\rewR$ is the preorder on $\basis$ defined by
$e\ordR e'$ if there exists a vector $u\in V$ such that there is a non
empty rewriting sequence $e'\transR u$ and $e\in\supp(u)$. The notation
$\ordR$ is the same than the one that we choose for the rewriting
preorder of a strategy since, even if they refer to different objects,
they are analogous: in fact $\ordS$ was defined as being the adaptation
of $\ordR$ to the case of parallel rewriting. Before proving the
diamond's lemma using strategies, we need the following preliminary
result.
\smallskip
