...
\newcommand\equivRx{\overset{*}{\leftrightarrow}_{R_{\prec \rho}}}
...
\begin{remark}
  By Proposition \ref{prop:vs_structure}, the decreasingness of a strategy $S$
  can be stated as the fact that for any rule $\rho = e \rewR v \in R$,
  $v \equivRx r_S(e)$, in other words that there exists a rewriting zig-zag
  between $v$ and $r_S(e)$ using only rewriting steps smaller than $\rho$.
  \todo{figure}

  This condition is analogous to the decreasingness condition introduced by van
  Ostroom for abstract rewriting systems \cite{van2008confluence}[Definition 2]. From this point of view, Theorem
  \ref{thm-S-conf} can be seen as a generalisation of van Ostroom's result [XX]
  to linear rewriting systems.
\end{remark}
