...
\begin{proof} \todo{adapter à $V/{\Span S}$}
...
\begin{definition}
$S$ is decreasing if there is an order $\prec$ or $R$ such that for any $\rho = (e,v) \in R$,
\[
v + \Span{R_{\prec \rho}} = r_S(e) + \Span{R_{\prec \rho}}
\]
\end{definition}

\begin{proposition}
$S$ is decreasing iff $R$ is $S$-confluent.
\end{proposition}
\begin{proof}
  If $R$ is $S$-confluent then we use the order $(e,r_S(e)) \leq (e,v)$. If
  $(e,v) \notin S$, $R_{<f} = S$, $v + \Span S = r_S(e) + \Span S$.

  If $S$ is decreasing, we proceed by induction on $R$ using $<$. Take $f=(e,v)$
  such that for ... (HR).  We want to show $e-v \in ker(\SNF)$. By induction,
  $R_{<f} \subset ker (\SNF)$. By decreasingness,
  $v - r_S(e) \in R_{<f} \subset ker(\SNF)$, and thus for $v - e$.
\end{proof}
...
As a consequence of the previous proposition, $V/\equivR = V/\Span{R}$ admits a
vector space structure, given by the operations
