...
\todo{Dire qu'on commence par rappeler des notions classiques sur les
  algèbres rationnelles de Weyl et les ordres monomiaux}

We fix a set $\{x_1,\cdots,x_n\}$ of indeterminates and we denote by
$\QXX$ the field of fractions of the polynomial algebra $\QX$ over
$\Q$. Let us introduce another set of variable
$\Delta:=\{\diff{1},\cdots,\diff{n}\}$, and let us denote by $\Delta^c$
the free commutative monoid over $\Delta$, \ie,
\[\Delta^c=\left\{\diff{1}^{\alpha_1}\cdots\diff{n}^{\alpha_n}\mid
\alpha_i\in\N\right\}.\]
Given $\alpha=(\alpha_1,\cdots,\alpha_n)\in\N^n$, we denote by
$\partial^{\alpha}:=\diff{1}^{\alpha_1}\cdots\diff{n}^{\alpha_n}$.

\begin{definition}
  The {\it rational Weyl algebra} over $\QXX$ is the set of polynomials
  over the inderminates $\Delta$ and coefficients in $\QXX$, subject to
  the following commutation rule:   
  \begin{equation}\label{equ:comm_rule}
    \diff{i}f=f\diff{i}+\frac{d}{dx_i}(f),\quad f\in\QXX,\quad
    1\leq i\leq n,
  \end{equation}
  where $d/dx_i:\QXX\to\QXX$ is the partial derivative operator with
...
$\alpha,\beta,\gamma\in\N^n$. 
\medskip

Given a set $F:=\{P_1,\cdots,P_k\}\subseteq\Weyl$
