...
\title{Parallel rewriting relations and the confluence property\\[0.2cm]
  of involutive sets in rational Weyl algebras\vspace{0.5cm}}
...
\begin{abstract}
  We study rewriting systems whose underlying set of terms admits vector
  space operations over a given field. We introduce parallel rewriting
  relations, that are rewriting relations compatible with vector space
  operations, as well as rewriting strategies, that consist in choosing
  one rewriting step for each reducible basis element of the vector
  space. Using these notions, we introduce the $S$-confluence property
  and show that it implies confluence. We deduce a proof of the diamond's
  lemma, based on strategies. We illustrate our general framework with
  rewriting systems over rational Weyl algebras, that are vector spaces
  over a field of rational functions. In particular, we show that
...
\end{abstract}
\noindent
\begin{small}\textbf{Keywords:} parallel rewriting, deterministic
  rewriting, linear rewriting.
\end{small}
...
The notion of well-formed rewriting step is specific to rewriting systems
over vector spaces and, as mentioned above, it does not satisfy suitable
compatibilities with the underlying algebraic operations. These two facts
are obstructions to the development of a categorical framework that we
hope to unify rewriting theories over algebraic structures. \todo{Mieux
  le dire}
\medskip
...
that consists in replacing each left-hand side of a rule $e$ occurring in
...
an involutive set in the rational Weyl algebra induces a confluent
...
A second research direction \todo{réécriture interne à une catégorie}.
\medskip
