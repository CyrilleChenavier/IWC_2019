...
\title{Parallel rewriting relations and the confluence property,\\[0.2cm]
 \Large  with application to involutive sets in rational Weyl algebras\vspace{0.5cm}}
...
\begin{small}\textbf{Keywords:} parallel rewriting, deterministic
  rewriting, linear rewriting, rewriting strategy.
\end{small}
...
vector spaces and, as mentioned above, it does not satisfy suitable
compatibilities with the underlying algebraic operations. In our view, these two
facts are a telltale sign that well-formed rewriting steps are incompatible with
the development of a categorical framework unifying rewriting theories over various
algebraic structures. \todo{Mieux le dire} \medskip
...
This work is focused on rewriting systems over vector spaces. A second research
direction is to extend it to other algebraic structures. This looks promising
since we do away with the notion of well-formed rewriting step, specific to the
vector space case. More generally, we hope to be able to extend our results to
an arbitrary category $\mathcal C$ (satisfying some suitable condition),
recovering abstract rewriting in the case where $\mathcal C$ is the category of
sets, and linear rewriting as presented in this work in the case where
$\mathcal C$ is the category of vector spaces. Instead of having a set or a
vector space of terms to be rewritten, one would then have an object of terms,
which would be an object of $\mathcal C$. 
