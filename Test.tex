...
extension of the order~$<$ to $V$ is simply called minimal.
...
in Section~\ref{sec:rewriting_systems_and_partial_derivative_equations}
for introducing our confluence criterion.
\medskip
...
basis element. Moreover, the seclected rewriting rule as to be compatible
with the order $<$.
...
\begin{definition}
  ...
  \item for every non minimal basis element $e$, there exists exactly one
    rewriting rule $e\rewR r_e$ in~$S$, morevoer we have $r_e<e$.
  \end{itemize}
\end{definition}
\smallskip
...
In order to illustrate our notions, we consider the following simple
running example.
...
We first introduce the $S$-confluence property.
...
Now, we introduce local $S$-confluence, which depends on a well-founded
...
\begin{definition}\label{def:decreasingness_property}
  ...
  strategy. We say that $\rewR$ is \emph{locally $S$-confluent with
    respect to} $\prec$ if for every $e\in\basis$ and every rewriting
  rule $e\rewR r$, then we have the confluence diagram:
  \[\large\begin{tikzcd}
...
\begin{example}\label{ex:end_to_example}
  Continuing with Example \ref{ex:h_norma_form}, let us consider the
