...
% Réécriture
\newcommand\rewR{\underset{R}{\to}}
\newcommand\rewh{\underset{h}{\to}}
\newcommand\transR{\overset{*}{\rewR}}
\newcommand\transh{\overset{*}{\rewh}}
\newcommand\equivR{\underset{R}{\overset{*}{\to}}}
\newcommand\equivh{\underset{h}{\overset{*}{\to}}}
...
In this section, we fix a \data\ $(\K,V,\basis,<)$. We first present our
...
rewriting rules of the form $e\rewR r$. The {\em linear extension} of $R$
...
the sum, $\lambda_i\neq 0$, $e_i\rewR r_i\in R$ and $e_i\notin\supp(v)$.
In the rest of Section~\ref{sec:deux}, the linear extension of $R$ is
also denoted by $\rewR$, and its closure under transitivity, symmetry and
sum is denoted by $\equivR$.

\begin{definition}
  A \emph{local strategy} $h$ for $R$ is the choice, for every non
  minimal basis element $e$, of a rewriting rule $h_e=e\rewR r_e$ such
  that $r_e<e$. A vector $v$ is said to be an \emph{$h$-normal form} if
  it is a normal form for the linear extension of $\rewh$.
\end{definition}
...
decomposed in a unique way as $\sum\lambda_ee+v'$, where $e'\in\supp(v')$
implies that $e'$ is minimal, and $e\in\supp(v)\setminus\supp(v')$ is
not. Hence, the linear extension of the local strategy $h$ rewrites $v$
as follows:
\begin{equation}\label{equ:well-formed_rewriting_step}
  h_v = \sum\lambda_ee+v'\rewh\sum\lambda_er_e+v',
\end{equation}
...
strategy $h$, $\rewh$ forms a terminating and confluent subrelation of
$\rewR$.
\medskip

We start in Definition~\ref{def:standardisation_property} and the
following propositions by relating the confluence of $\rewR$ to
properties on $h$. Then, in Theorem~\ref{thm:confluence_criterion}, we
prove a confluence criterion to check whether $R$ satisfies
Definition~\ref{def:standardisation_property}.

\begin{definition}\label{def:standardisation_property}
  Given a local strategy $h$, we say that $R$ is \emph{h-confluent} if
  for every rewrite rule $e\rewR r\in R$, we have $H(e-r)=0$.
\end{definition}
\smallskip
...
\begin{example}
  ...
  \]
  andso that $R$ is $h$-confluent. If we replace the rule $c\rewR b-d$ by
  $c\rewR b$, we get $H(c)=c$ and $H(b)=c+d$, so $R$ is not $h$-confluent
  anymore. 
\end{example}
\smallskip
...
\begin{proposition}\label{prop:h_and_R_confluence}
  ...
  \begin{enumerate}
  \item\label{it:equationnal_theory} If $R$ is $h$-confluent, then
    $u\equivR v$ if and only if $H(u-v)=0$.
  \item\label{it:h-conf_implies_conf} If $R$ is $h$-confluent then
    $\rewR$ is confluent.
  \end{enumerate}
\end{proposition}
...
\begin{proof}
  ...
  operations, we get one implication. Reciprocally, if $H(u-v)=0$, then
  by definition of $H$ we have $u\equivh v$, and in particular
  $u\equivR v$. \todo{Il me semble qu'il y a un problème car le rôle de
    $u$ et $v$ dans $H(u-v)=0$ est symétrique}
  \smallskip
  
  Let us show Point~\ref{it:h-conf_implies_conf}. Let $v,v_1,v_2\in V$ be
  such that $v\transR v_i$, for $i=1,2$. By~\ref{it:equationnal_theory},
  $H(v_1)=H(v_2)$, and by denoting by $u$ this common value. That proves
  that $\rewR$ is confluent.
\end{proof}
\medskip

Note that the previous proposition is a sufficient but not a necessary
condition: taking $\basis$ to be the integers, with the rewriting
relation induced by $n\rewR n+1$ is confluent, but there exist no local
strategy $h$ such that $R$ is $h$-confluent.
\medskip
