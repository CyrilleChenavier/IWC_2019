...
\end{itemize}
In the sequel, such a tuple will be called a \data. Given a \data, we
simply say ground field, vectors and basis elements for $\K$, elements of
$V$ and $\basis$, respectively. Moreover, every vector $v$ admits a
unique finite decomposition with coefficients in the ground field
\begin{equation}\label{equ:vector_decompo}
  v=\sum\lambda_ie_i,\quad\lambda_i\neq 0
\end{equation}
with respect to the basis $\basis$. A basis element which is minimal for
$<$ is simply called minimal. The set of basis elements which
appear in the decompositon \eqref{equ:vector_decompo} is called the
{\it support} of $v$, and is written $\lm(v)$. We extend $<$ into the
multiset order on $V$: for any $u,v \in V$, we have $u<v$ if
$\supp(u)\neq\supp(v)$ and for any $e\in\supp(u)\setminus\supp(v)$, there
exists $e'\in\supp(v)\setminus\supp(u)$ such that $e'>e$. Notice that
this order coincides with the initial one on basis elements, so that we
can denote it by $<$ without ambiguity.
...
We fix a set $R\subseteq\basis\times V$ which represents rewrite rules of
the form $e\rewR r$. The set $R$ induces a rewriting relation on $V$,
where many basis elements are reduced at once, defined as follows:
\begin{small}
  \begin{equation}\label{equ:rewriting_step}
    \sum_i\lambda_ie_i+v\rewR\sum_i\lambda_ir_i+v,
  \end{equation}
\end{small}
where $v$ is any element of $V$, and for any $e_i\in\basis$ appearing in
the sum, $\lambda_i\neq 0$, $e_i\rewR r_i\in R$ and $e_i\notin\supp(v)$.
Finally we denote by $\equivR$ the closure of $\rewR$ under transitivity,
symmetry and sum.

\begin{definition}
  A \emph{local strategy} $h$ for $R$ is the choice, for every non
  minimal basis element $e$ which is not minimal, of a rewriting rule
  $h_e=e\rewR r_e$ such that $r_e<e$. 
\end{definition}

In the rest of this section, we suppose chosen such a local strategy $h$
(note that such an $h$ may not exist). Any vector $v$ can be decomposed
in a unique way as $\sum\lambda_ee+v'$, where $e'\in\supp(v')$ implies
that $e'$ is minimal, and $e\in\supp(v)\setminus\supp(v')$ is not. We
define a rewriting relation $\rewh$ as follows:
\begin{small}
  \begin{equation}\label{equ:well-formed_rewriting_step}
    h_v = \sum\lambda_ee+v'\rewh\sum\lambda_er_e+v',
  \end{equation}
\end{small}
where for every $e$, $h_e=e\rewR r_e$. Note in particular that if $e$ is
minimal, then $h_e=e\to e$ is the identity on $e$.
