...
In Theorem~\ref{thm:confluence_criterion}, we prove our confluence
criterion for $\rewR$. For that, we introduce two notions, that we call
$S$-confluence and local $S$-confluence, and prove that the latter
implies the former, which itselve implies confluence of $\rewR$.
...
\begin{example}\label{ex:end_to_example}
  Continuing with Example \ref{ex:h_norma_form}, let us define the order
  $\preceq$ on $R$ as follows: $(a\rewR b), (b\rewR c+d)\prec
  (c\rewR b-d)$. This is guided by the heuristic that rules advancing
  towards an $S$-normal form should be favored over rules that do not:
  here $c$ is an $S$-normal form so its rewriting rule is large for
  $\prec$. The following diagrams show that $\rewR$ is locally
  $S$-confluent:
  \[
  \xymatrix @C = 2.5em @R = 2.5em{
    a \ar [r] _{R}  
